...
class EditSchemasGenerateRequestFile(UncheckedBaseModel):
    """
    ...
    """
    If you already have an Extend file id (for instance from running a workflow or a previous file upload) then you can use that file id when generating an edit schema. The file id will start with "file_". One of `fileUrl` or `fileId` must be provided.
