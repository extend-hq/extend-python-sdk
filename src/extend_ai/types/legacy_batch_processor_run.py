...
class LegacyBatchProcessorRun(UncheckedBaseModel):
    ...
    metrics: LegacyBatchProcessorRunMetrics
    status: LegacyBatchProcessorRunStatus = pydantic.Field()
    """
    ...
    * `"FAILED"` - The batch processor run encountered an error
    ...
    source: LegacyBatchProcessorRunSource = pydantic.Field()
    """
    ...
    * `"STUDIO"` - The batch processor run was made for a processor in Studio. The `sourceId` will be the ID of the processor (e.g., `"ex_1234"`)
    ...
    options: LegacyBatchProcessorRunOptions = pydantic.Field()
    """
