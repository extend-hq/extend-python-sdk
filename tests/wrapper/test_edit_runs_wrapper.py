...
def create_mock_edit_run(status: str = "PROCESSED", run_id: str = "edit_run_test123"):
    """Create a mock EditRun response."""
    mock_run = MagicMock()
    mock_run.id = run_id
    mock_run.status = status
    mock_run.object = "edit_run"
    return mock_run
...
def create_mock_create_response(status: str = "PROCESSING"):
    ...
    response = MagicMock()
    response.edit_run = create_mock_edit_run(status)
    return response
...
class TestEditRunsWrapperCreateAndPoll:
    ...
    def test_creates_and_polls_until_processed(self):
        ...
        assert result.edit_run.status == "PROCESSED"
    ...
    def test_handles_failed_status_as_terminal(self):
        ...
        assert result.edit_run.status == "FAILED"
