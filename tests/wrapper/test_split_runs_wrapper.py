...
def create_mock_split_run(status: str = "PROCESSED", run_id: str = "split_run_test123"):
    """Create a mock SplitRun response."""
    mock_run = MagicMock()
    mock_run.id = run_id
    mock_run.status = status
    mock_run.object = "split_run"
    return mock_run
...
def create_mock_create_response(status: str = "PROCESSING"):
    ...
    response = MagicMock()
    response.split_run = create_mock_split_run(status)
    return response
...
class TestSplitRunsWrapperCreateAndPoll:
    """Tests for SplitRunsWrapper.create_and_poll method."""

    def setup_method(self):
        """Set up test fixtures."""
        from extend_ai.wrapper.resources.split_runs import SplitRunsWrapper

        self.wrapper = MagicMock(spec=SplitRunsWrapper)
        self.wrapper.create = MagicMock()
        self.wrapper.retrieve = MagicMock()

        self.wrapper.create_and_poll = SplitRunsWrapper.create_and_poll.__get__(
            self.wrapper, SplitRunsWrapper
        )
    ...
    def test_creates_and_polls_until_processed(self):
        ...
        assert result.split_run.status == "PROCESSED"
    ...
    def test_handles_failed_status_as_terminal(self):
        ...
        assert result.split_run.status == "FAILED"
    ...
    def test_handles_cancelled_status_as_terminal(self):
        ...
        assert result.split_run.status == "CANCELLED"
